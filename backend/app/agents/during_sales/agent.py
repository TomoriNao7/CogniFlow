"""During-sales Agent — LangGraph workflow orchestrating RAG + Tools + LLM generation."""
...
from app.agents.during_sales.prompt import DURING_SALES_SYSTEM_PROMPT
from app.agents.during_sales.tools import register_during_sales_tools
...
class DuringSalesAgent:
    """LangGraph-based during-sales agent.
    ...
    def __init__(self) -> None:
        global _registered
        if not _registered:
            register_during_sales_tools(agent_id=self.AGENT_ID)
            _registered = True
    ...
    async def _decide_tools(self, state: AgentState) -> AgentState:
        tools_schema = tool_registry.to_openai_tools(agent_id=self.AGENT_ID)
        if not tools_schema:
            state["tool_calls"] = []
            return state
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "你是售中客服决策助手。根据用户问题和已有知识库内容，判断是否需要调用工具获取实时数据。\n"
                    "需要实时数据的场景：查询订单状态、查询支付信息、修改地址、申请发票。\n"
                    "不需要工具的场景：支付方式咨询、发票政策说明、地址修改规则说明（知识库已覆盖）。\n"
        ...
        resp = await self.llm.chat.completions.create(
            model=settings.during_sales_model,
            messages=messages,
            tools=tools_schema,
            tool_choice="auto",
            temperature=0.1,
    ...
    async def _call_tools(self, state: AgentState) -> AgentState:
        ...
        for tc in state["tool_calls"]:
            ...
            try:
                ...
                circuit_check(tc["name"]).record_success()
                results.append({
                    "tool": tc["name"],
                ...
                circuit_check(tc["name"]).record_failure()
    ...
    async def _generate(self, state: AgentState) -> AgentState:
        ...
        system_prompt = DURING_SALES_SYSTEM_PROMPT.replace(
...
# Singleton
during_sales_agent = DuringSalesAgent()
