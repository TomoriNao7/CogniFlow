"""After-sales Agent — LangGraph workflow orchestrating RAG + Tools + LLM generation."""
...
from app.agents.after_sales.prompt import AFTER_SALES_SYSTEM_PROMPT
from app.agents.after_sales.tools import register_after_sales_tools
...
class AfterSalesAgent:
    """LangGraph-based after-sales agent.
    ...
    def __init__(self) -> None:
        global _registered
        if not _registered:
            register_after_sales_tools(agent_id=self.AGENT_ID)
            _registered = True
    ...
    async def _decide_tools(self, state: AgentState) -> AgentState:
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "你是售后客服决策助手。根据用户问题和已有知识库内容，判断是否需要调用工具获取实时数据。\n"
                    "需要实时数据的场景：查询物流信息、查询订单详情、发起退款、查询退款进度。\n"
                    "不需要工具的场景：退换货政策说明、投诉流程咨询（知识库已覆盖）。\n"
        ...
        resp = await self.llm.chat.completions.create(
            model=settings.after_sales_model,
            messages=messages,
            tools=tools_schema,
            tool_choice="auto",
            temperature=0.1,
    ...
    async def _generate(self, state: AgentState) -> AgentState:
        ...
        system_prompt = AFTER_SALES_SYSTEM_PROMPT.replace(
        ...
        state["trace"]["model"] = settings.after_sales_model
